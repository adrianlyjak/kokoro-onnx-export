...
from kokoro_onnx.util import get_onnx_inputs, load_vocab, mel_spectrogram_distance
...
def run_quantization_trials(
    ...
    tensors_range = extract_tensors_range(
        ...
        for node in remaining_trials:
            ...
            if not should_quantize:
                with open(output_file, "a") as f:
                    f.write(f"{node.name},quant,{node.op_type},N/A\n")
            ...
            with open(output_file, "a") as f:
                f.write(f"{node.name},quant,{node.op_type},{mel_dist}\n")
    ...
    with Progress(
        ...
        for node in remaining_trials:
            ...
            if not should_convert:
                with open(output_file, "a") as f:
                    f.write(f"{node.name},cast,{node.op_type},N/A\n")
                progress.advance(task)
                continue
            ...
            # Append result to CSV
            with open(output_file, "a") as f:
                f.write(f"{node.name},cast,{node.op_type},{mel_dist}\n")
...
QUANT_RULES = [
    ...
    OpQuantizationRule("ConvTranspose", False, "int8", "int8"),
...
def select_node_datatypes(
    ...
    for name, trials in results.items():
        ...
        specs.append(
            NodeQuantSpec(
                name, weights_type, activations_type, q_operator=rule.q_operator
            )
        )
...
def estimate_quantized_size(
    ...
    # Process nodes (for activation storage)
    for node in model.graph.node:
        if node.name in specs_by_name:
            spec = specs_by_name[node.name]
            bytes_per_element = get_bytes_per_element(spec.activations_type)
        else:
            bytes_per_element = 4  # Default to float32

        # Estimate activation size based on output shapes
        # This is a rough estimate since actual runtime memory usage may vary
        for output in node.output:
            # Try to find value_info for shape information
            shape_info = next(
                (vi for vi in model.graph.value_info if vi.name == output), None
            )
            if shape_info:
                dims = [dim.dim_value for dim in shape_info.type.tensor_type.shape.dim]
                size = np.prod(dims) * bytes_per_element
                total_size += size
